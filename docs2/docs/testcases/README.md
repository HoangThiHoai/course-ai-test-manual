...
|---|---|---|---|---|---|---|---|---|---|
| Đăng nhập / Xác thực | `CRM_LOGIN_TC_` | `001` → `053` | `054` | 53 | Web 53 | GỘP | 40/40 REQ trong phạm vi | [TEST_CASES_LOGIN_SUMMARY.md](login/TEST_CASES_LOGIN_SUMMARY.md) | 21-09-2026 |
...
|---|---|---|---|
| `LOGIN` | **GỘP** | Sinh mới toàn bộ 21-09-2026 ở độ hạt GỘP — 53 TC · **101 biến thể**, 2 part (vượt ngưỡng 50) | Bộ 51 TC trước đó đã bị xoá, chưa từng commit → không tra được bằng git. Ánh xạ mã cũ → mới cho bug: mục *Ánh xạ TC ID cũ → mới* trong index module |
...
| Ngày | Thay đổi |
|---|---|
| 21-09-2026 | **`LOGIN` sinh mới toàn bộ** bằng `/generate-testcases-from-requirements` Mode QUICK, độ hạt GỘP, rủi ro Cao → Đầy đủ — **53 TC · 101 biến thể**, 2 part ở `login/web/parts/`, dải `CRM_LOGIN_TC_001` → `053`, phủ 40/40 REQ. Bộ 51 TC cũ đã bị xoá khỏi đĩa trước lượt này và chưa từng commit. ⚠️ 6 bug ở `docs/bugs/login/web/` và 2 execution report cũ còn trỏ **mã TC cũ** — bảng ánh xạ ở index module |
