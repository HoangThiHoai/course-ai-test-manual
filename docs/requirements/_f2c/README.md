...
| **Năng lực kiểm thử của QA** | Chốt 2026-09-18 — dùng cho nhánh Vòng 3 của **mọi** bộ TC trong namespace `_f2c/`:<br>• Gọi API: ✅<br>• Truy vấn CSDL: ❌ — Dev CMS/Backend chịu trách nhiệm xác minh<br>• Kiểm tầng tích hợp (Seller/Admin CMS, ĐVVC, Zalopay): ❌ (không có tài khoản test Seller/Admin) — Dev CMS chịu trách nhiệm<br>• Xem nhật ký hoạt động: ❌ — Dev/Ops chịu trách nhiệm<br>• DevTools trình duyệt: ✅ |
...
|---|---|---|---|---|---|
| AMB-02 | 🔴 High | `ORDBUY` — Trạng thái "Hoàn tiền thành công/thất bại" phụ thuộc Admin import CMS, nhưng SRS tự ghi tính năng import **chưa được xây**. Xác nhận độc lập ở cả 3 SRS (Buyer/Seller/Admin — xem AMB-09 và mục 4b của `ORDADM`). **Cách xử lý đã được PO xác nhận (2026-09-18): giữ nguyên assumption BLOCKED** — TC liên quan gắn nhãn `⚪ BLOCKED chờ CMS import`, test qua chỉnh DB trực tiếp nếu QA có quyền. Vẫn còn treo vì bản thân tính năng CMS import **chưa được xây** | Block toàn bộ TC liên quan Hoàn tiền thành công/thất bại, cả 3 module | Dev CMS (xây tính năng) | [ORDBUY](ORDBUY/analysis/analysis_SRS-ORDBUY-V3.md) · [ORDSEL](ORDSEL/analysis/analysis_SRS-ORDSEL-V4.md) · [ORDADM](ORDADM/analysis/analysis_SRS-ORDADM-V1.1.md) |
| AMB-05 | ✅ Đã giải quyết | Số lần tối đa Buyer được gửi yêu cầu Trả hàng/Hoàn tiền = **2 lần** — **xác nhận trực tiếp bởi PO ngày 2026-09-18**, đồng nhất với xác nhận chéo trước đó ở SRS Seller và Admin | Không còn block TC | — (đã đóng 2026-09-18) | [ORDBUY](ORDBUY/analysis/analysis_SRS-ORDBUY-V3.md) · [ORDSEL](ORDSEL/analysis/analysis_SRS-ORDSEL-V4.md) AMB-10 · [ORDADM](ORDADM/analysis/analysis_SRS-ORDADM-V1.1.md) mục 4b |
| AMB-09 | 🔴 High | `ORDSEL` — xem AMB-02, cùng 1 điểm chặn | — | Dev CMS | [ORDSEL/analysis/analysis_SRS-ORDSEL-V4.md](ORDSEL/analysis/analysis_SRS-ORDSEL-V4.md) |
...
| AMB-01 · AMB-03 · AMB-04 · AMB-06 · AMB-07 · AMB-08 | ✅ Đã giải quyết | `ORDBUY` — toàn bộ 6 ambiguity còn lại của đợt phân tích Ver 3.0.0 đã được **PO xác nhận đúng ngày 2026-09-18** (`AMB-06`, `AMB-07` có nội dung câu trả lời khác với assumption tạm ban đầu — xem chi tiết mục 7.1 của tài liệu phân tích) | Không còn block TC | — (đã đóng 2026-09-18) | [ORDBUY/analysis/analysis_SRS-ORDBUY-V3.md](ORDBUY/analysis/analysis_SRS-ORDBUY-V3.md) mục 7.1 |
...
| Ngày | Thay đổi | Lý do |
|---|---|---|
| 2026-09-18 | Chốt **Năng lực kiểm thử của QA** (bảng thuộc tính đầu file): Gọi API ✅, còn lại (CSDL/tích hợp/nhật ký) ❌ | Trả lời của user trước khi sinh TC cho `ORDBUY` — dùng chung cho Vòng 3 (Technical) của mọi module `_f2c/`, không hỏi lại theo từng module |
| 2026-09-18 | Đóng toàn bộ `AMB-01` → `AMB-08` của `ORDBUY` (✅ Đã giải quyết); hạ `AMB-02` xuống chỉ còn treo phần "tính năng CMS import chưa xây" (cách xử lý đã chốt); cập nhật bảng mục 1 và mục 3 | PO xác nhận trực tiếp qua chat toàn bộ 8 ambiguity của đợt phân tích `ORDBUY` Ver 3.0.0 |
